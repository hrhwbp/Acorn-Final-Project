...
public interface AnnoInter {
	...
	@Update("update board set b_content=#{b_content}")
	...
	@Update("update member set m_name = #{m_name}, m_bdate = #{m_bdate}, m_email = #{m_emali}, m_gender=#{m_gender}, m_password=#{m_password}")
	...
	boolean follow(FollowBean bean) throws DataAccessException;	//f_sno°¡ f_mno¸¦ follow
	...
	@Update("update reply set r_content = #{r_content}")
	...
	@Update("update wishlist set w_pname=#{w_pname}, w_price=#{w_price}, w_image=#{w_image}, w_addr=#{w_addr}, w_detail=#{w_detail}")
